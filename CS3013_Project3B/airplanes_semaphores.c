...
//create pthread array and binary semaphores
pthread_t tid[25];
		...
		for(i = 0; i < airplanes[planeID].fuel; i++) {
			if((emergencies > 0 && airplanes[planeID].emergency == 0) || (airplanes[planeID].priority != 1)) {
				...
				if(runways[0].occupied == false){
					//if(sem_wait(&sem1) == 0){ // check semaphores	
						sem_wait(&sem1);
					...
					//if(sem_post(&sem2) == 0){ // check semaphores	
						sem_wait(&sem2);
					...
					//if(sem_post(&sem3) == 0){ // check semaphores
						sem_wait(&sem3);
