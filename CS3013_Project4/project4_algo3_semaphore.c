...
// This file implements the LIFO algorithm.
...
vAddr create_page(){
	...
	sem_post(&page_create);		// Do a sem_wait for mutual exclusion
	return address; 
}
...
// If an empty RAM slot was not find then, run the page evict algorithm to evict a page from RAM.
	...
	// Run the LIFO algorithm to find the least recently used page in RAM
	int RAM_ind = LIFO(RAM_Capacity);	
...
uint32_t get_value(vAddr address, int *valid){
	...
	if (page_table[address].valid == 0){
		...
		valid = NULL;
		sem_post(&value_get);		// Do a sem_wait for mutual exclusion
		return 0;
	}
...
void store_value (vAddr address, uint32_t *value){
	...
	sem_post(&value_store);		// Do a sem_wait for mutual exclusion							
}
...
void free_page (vAddr address){
	...
	sem_post(&page_free);		// Do a sem_wait for mutual exclusion
}
...
int LIFO(int capacity){
	...
	if (capacity == RAM_Capacity){
		for (modind = 0; modind < capacity; modind++){
			for (pgind = 0; pgind < Page_Table_Capacity; pgind++){
				if ((page_table[pgind].flag == 1) && (page_table[pgind].valid == 1)){
					flag_count = page_table[pgind].flag;
					index = modind;
				}
			}
		}
	...
	printf("The chosen page based on LIFO algorithm is %d \n", index);	
	return index;					
}
